...
def build():
 for n in ['dist','preview']:(R/n).mkdir(exist_ok=True)
 f=json.loads((R/'assets/native-format.json').read_text())
 for pet in ['sol','luna']:
  out=R/'pets'/pet;out.mkdir(parents=True,exist_ok=True)
  for n in ['pet.json','spritesheet.webp']:shutil.copy2(R/'assets/approved'/pet/n,out/n)
 ...
 zip_files(R/'dist'/f'Sol-Luna-v{V}-install.zip',[(f'{p}/{n}',(R/'pets'/p/n).read_bytes()) for p in ['sol','luna'] for n in ['pet.json','spritesheet.webp']])
