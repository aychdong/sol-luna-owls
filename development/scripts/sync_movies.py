...
R=Path(__file__).resolve().parents[2];v=(R/'VERSION').read_text().strip();dest=a.destination;stamp=datetime.datetime.now().strftime('%Y%m%d-%H%M%S');stage=dest.parent/('.Sol-Luna-staging-'+stamp)
...
shutil.copy2(R/'dist'/f'Sol-Luna-v{v}-user-preview.html',stage/'预览/动画预览.html')
...
(R/'development/reports/movies-delivery.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');print(json.dumps(report,ensure_ascii=False,indent=2))
