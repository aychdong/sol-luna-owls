...
const ctx=vm.createContext({document:{documentElement:{},body:element(),createElement:element,getElementById:id=>{assert(ids[id],id);return ids[id]},querySelectorAll:()=>labels},
  matchMedia(){return {matches:false}},setTimeout(fn,ms){callback=fn;duration=ms;return 1},clearTimeout(){callback=null}});
...
assert.equal(get('selected'),0);assert.equal(get('frame'),0);assert.equal(ids['sol-new'].style.backgroundPosition,'0px 0px');
ids.action.onchange({target:{value:9}});
...
ids.action.onchange({target:{value:9}});ids.play.onclick();for(let i=0;i<16;i++)callback();assert.equal(get('frame'),0);
ids.theme.onclick();assert.equal(ids.theme.attrs['aria-pressed'],'true');
const links=[...html.matchAll(/(?:href|src)="([^"]+)"|url\('([^']+)'\)/g)].map(m=>m[1]||m[2]);
for(const link of links.filter(x=>!x.startsWith('https:')&&!x.startsWith('#')))assert(fs.existsSync(path.resolve(root,decodeURI(link))),`Missing local asset: ${link}`);
const report={status:'passed',method:'Node VM control checks and local file checks; no browser/native UI test',checks:['10 V1 state choices','16 direction mappings','native frame durations','loop wrap','slow playback','paused stepping','bilingual labels','96 px size','dark background control','all local asset paths resolve (remote links verified separately)']};
fs.writeFileSync(path.join(root,'development/reports/preview-checks.json'),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));
