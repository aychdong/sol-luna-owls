...
R=Path(__file__).resolve().parents[2];V=(R/'VERSION').read_text().strip();F=json.loads((R/'development/assets/native-format.json').read_text());records=[]
D=Path(os.environ.get('SOL_LUNA_DIST',str(R/'dist')))
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
for pet in ['sol','luna']:
 folder=R/'pets'/pet
 for n in ['pet.json','spritesheet.webp']:assert (folder/n).read_bytes()==(R/'development/assets/approved'/pet/n).read_bytes()
 ...
 a=np.array(im);r3=np.array(Image.open(R/'development/assets/baseline-r3'/pet/'spritesheet.webp').convert('RGBA'));v1=np.array(Image.open(R/'development/assets/baselines/v1-original'/pet/'spritesheet.webp').convert('RGBA'))
 def changed(b):
...
with zipfile.ZipFile(D/f'Sol-Luna-v{V}-install.zip') as z:
...
for e in json.loads((R/'development/assets/provenance.json').read_text()):assert sha(R/e['path'])==e['sha256']
...
(R/'development/reports/package-validation.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))
